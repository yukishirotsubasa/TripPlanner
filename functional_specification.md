...
- [V] 能夠從地圖搜尋列找到景點，並成功新增至指定天數中。
- [V] 能夠透過滑鼠或手指拖曳景點卡片，並成功改變在該天的排序順序。
- [V] 設定停留時間後，能總結顯示當天總共耗費的總時間。
- [V] 點擊「產生分享連結」，可複製一串長網址，並在新的無痕視窗開啟後，能成功看到剛才排好的行程且無法進行修改。
- [V] 使用 `npm run build` 無錯誤且順利編譯。
